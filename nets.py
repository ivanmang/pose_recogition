...
class MyVGG11(nn.Module):
    def __init__(self, in_ch, num_classes):
        ...
        self.conv_block1 = vgg_block_single(in_ch, 128)
        self.conv_block2 = vgg_block_single(128, 256)

        self.conv_block3 = vgg_block_double(256, 256)
        #self.conv_block4 = vgg_block_double(256, 512)
        #self.conv_block5 = vgg_block_double(512, 512)

        # self.fc_layers = nn.Sequential(
        #     nn.Linear(512 * 32*32, 4096), nn.ReLU(inplace=T  rue), nn.Dropout(),
        #     nn.Linear(4096, 4096), nn.ReLU(inplace=True), nn.Dropout(),
        #     nn.Linear(4096, num_classes)
        # )

        self.fc_layers = nn.Sequential(
            nn.Linear(12544, 64), nn.ReLU(inplace=True), nn.Dropout(),
